...
- UI: `components/help/HdsdLink.tsx` — `{moduleSlug, topicSlug?}` → `<a href="/docs/..." target="_blank">HDSD</a>`, không còn `mode`/phân biệt desktop-mobile (mở tab mới luôn an toàn). `HelpArticle.tsx`/`HelpModuleIndex.tsx`/`HelpLanding.tsx` prop/URL-driven (không còn context). `HelpBreadcrumb.tsx` dùng `<Link>` thật. `help.css` (`animate-fade-in-once` khi đổi bài, tôn trọng `prefers-reduced-motion`).
