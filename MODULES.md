...
- Nội dung: `frontend/src/content/help/<module-slug>/<topic-slug>.md` — 9 module có bài (`paymentRequests`, `reconciliation`, `reconCard`, `module3`, `module4`, `revenueLedger`, `bc01`, `bc02`, `bc03`), 30 bài — đang mở rộng ra toàn bộ ~22 module theo yêu cầu 27/07. `moduleSlug` = đúng `ViewId` trong `MainPage.tsx`. Frontmatter chỉ 3 field: `title`/`order`/`audience`. **Bắt buộc ≥1 ảnh minh họa/bài** — lưu `frontend/public/docs-images/<module>/<topic>-N.png`, nhúng `![alt](/docs-images/...)`; enforcement ở `content/help/screenshots.test.ts` (mảng `NO_SCREENSHOT_YET` liệt kê bài chưa có ảnh, tự dọn dần).
...
- Phạm vi điểm chèn: 24 điểm cũ (6 module ưu tiên: Payment Requests, Đối soát, Kích hoạt, Xuất hóa đơn, Sổ doanh thu) + đang mở rộng thêm ~13 chỗ hổng NGAY TRONG 6 module đó (toolbar/bảng chính trước đây chưa gắn) + ~29 điểm mới ở 12 module trước đây chưa đụng (CRM sync, Dashboard Sale, Gateway Sync, Zalo×3, DingTalk×3, Auth Accounts, Permissions...). Nguồn sự thật: `grep -rn "<HdsdLink" frontend/src`.
