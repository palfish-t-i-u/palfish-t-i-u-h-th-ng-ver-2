...
Kế hoạch đầy đủ: `docs/plans/HANDOFF_HDSD_DOCS_ROUTE_PIVOT_2026-07-27.md` (thay thế 4 doc HDSD ngày 26/07 — đã đánh dấu SUPERSEDED). **Không còn in-app** — docs sống ở route riêng `/docs/*`, HDSD luôn mở tab mới (`target="_blank"`), không gate theo quyền (vẫn cần login qua `ProtectedRoute` chung với `/`).

- Nội dung: `frontend/src/content/help/<module-slug>/<topic-slug>.md` — 21 module có bài (`paymentRequests`, `reconciliation`, `reconCard`, `module3`, `module4`, `module5`, `module6`, `revenueLedger`, `bc01`, `bc02`, `bc03`, `dashboard`, `gatewaySync`, `authAccounts`, `permissions`, `zaloConfig`, `zaloGroups`, `zaloOutbox`, `dingtalkConfig`, `dingtalkGroups`, `dingtalkOutbox`), 52 bài, 59 điểm chèn `HdsdLink`. `moduleSlug` = đúng `ViewId` trong `MainPage.tsx`. Frontmatter chỉ 3 field: `title`/`order`/`audience`. **Bắt buộc ≥1 ảnh minh họa/bài** — lưu `frontend/public/docs-images/<module>/<topic>-N.png`, nhúng `![alt](/docs-images/...)`, chụp bằng `e2e/docs-screenshots.spec.ts` trên sandbox thật (`npx playwright test e2e/docs-screenshots.spec.ts --config playwright.sandbox.config.ts`); enforcement ở `content/help/screenshots.test.ts` (mảng `NO_SCREENSHOT_YET` — còn 12 bài cũ chưa có ảnh vì thao tác chụp sẽ mutate dữ liệu sandbox dùng chung team (huỷ/chuyển giao PR, ghép giao dịch, sửa dòng sổ doanh thu...) hoặc cần trạng thái nghiệp vụ đặc thù — cần quyết định riêng trước khi chụp).
- Loader: `content/help/index.ts` — `import.meta.glob("./**/*.md", {query:"?raw",import:"default",eager:true})` + `parseFrontmatter()` tự viết (KHÔNG `gray-matter` — gọi `Buffer`, crash runtime). **Không import module này từ bất kỳ file eager-load nào** (`MainPage.tsx`/`AppShell.tsx`) — nếu không, toàn bộ text markdown sẽ lọt vào bundle chính bất kể `HelpArticle`/`DocsLayout` có lazy hay không (xem `docs/learnings/vite-eager-glob-bundle-leak.md`). `content/help/moduleLabels.ts` — nhãn hiển thị, tách riêng tránh circular import.
- Route: `App.tsx` có `<Route path="/docs/*">` dùng chung `ProtectedRoute`, lazy qua `frontend/src/lib/lazyRetry.ts` (tách từ `MainPage.tsx` để dùng chung). `pages/docs/DocsLayout.tsx` — shell 2 cột, tự có `<Routes>` con (`index`/`:moduleSlug`/`:moduleSlug/:topicSlug`). `pages/docs/DocsSidebar.tsx` — cây module→topic; dùng `useLocation()` parse URL để biết module/topic đang active, **không dùng `useParams()`** (sidebar là sibling của `<Routes>` lồng bên trong, không phải route con nên không thấy params — xem `docs/learnings/react-router-useparams-sibling-scope.md`).
- UI: `components/help/HdsdLink.tsx` — `{moduleSlug, topicSlug?}` → `<a href="/docs/..." target="_blank">HDSD</a>`, không còn `mode`/phân biệt desktop-mobile (mở tab mới luôn an toàn). `HelpArticle.tsx`/`HelpModuleIndex.tsx`/`HelpLanding.tsx` prop/URL-driven (không còn context). `HelpArticle` tự sinh `id` cho h2/h3 (slugify bỏ dấu tiếng Việt) để bài viết làm được mục lục anchor `[...](#...)`, và tự `scrollIntoView` khi hash đổi — React Router nuốt fragment-scroll mặc định của trình duyệt nên KHÔNG bỏ effect này. `HelpBreadcrumb.tsx` dùng `<Link>` thật. `help.css` (`animate-fade-in-once` khi đổi bài, tôn trọng `prefers-reduced-motion`).
- Sửa file chung: `AppShell.tsx` (link tĩnh `/docs` cuối sidebar, `HdsdLink` không điều kiện ở header), `MainPage.tsx` (không còn `"help"` ViewId/state/Provider gì cả — header luôn truyền `helpModuleSlug={activeView}`).
- Phạm vi điểm chèn: đã phủ toàn bộ 21 module có docs (Payment Requests, Đối soát, Kích hoạt, Xuất hóa đơn, Sổ doanh thu, CRM sync, Dashboard Sale, Gateway Sync, Zalo×3, DingTalk×3, Auth Accounts, Permissions...) — 59 điểm `HdsdLink`. Nguồn sự thật: `grep -rn "<HdsdLink" frontend/src`.

## 13. Phiếu lương (M4)

> App = tầng hiển thị thuần (KHÔNG tính lại). Sheet HR tính → Gate POST → app lưu + hiển thị.
...
- M3 Apps Script: `docs/apps-script/BangLuong.gs` (bảng lương), `DoiSoatLuong.gs` (đối soát), `BangTinhThue.gs` (tham chiếu thuế), `BotTinhLuong.gs` (Cloud Run job + modal dialog tính lương), `DialogTinhLuong.html` (modal thông báo tính lương 2 trạng thái)
- BQ auth: `docs/apps-script/BqAuth.gs` — gọi BigQuery qua **Service Account** (JWT→token, REST) thay Advanced Service; ai chạy cũng được (không cần cấp quyền BQ cho người dùng). Key ở Script Properties `SA_CLIENT_EMAIL`/`SA_PRIVATE_KEY`. Dùng bởi `BangLuong.gs` + `BangTinhThue.gs` (`bqQuery_`, `bqLoadJson_`)
