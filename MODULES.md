...
- Nội dung: `frontend/src/content/help/<module-slug>/<topic-slug>.md` — 9 module (`paymentRequests`, `reconciliation`, `reconCard`, `module3`, `module4`, `revenueLedger`, `bc01`, `bc02`, `bc03`), 30 bài. `moduleSlug` = đúng `ViewId` trong `MainPage.tsx`. Frontmatter chỉ 3 field: `title`/`order`/`audience`.
...
- UI: `components/help/HdsdLink.tsx` (link "HDSD"; mode `"topic"` nhảy thẳng bài viết. mode `"module"` phân nhánh theo thiết bị: desktop chỉ mở cây sidebar — không đổi `activeView`, không mất state đang thao tác dở; mobile gọi `goToModuleIndex` nhảy vào mục lục module vì `MobileNavSheet.tsx` KHÔNG render `HelpNavTree` nên "mở cây" là no-op. Đánh đổi có chủ đích: trên mobile không còn nút HDSD "an toàn không điều hướng"), `HelpArticle.tsx`/`HelpModuleIndex.tsx`/`HelpLanding.tsx` (lazy-load qua `MainPage.tsx`'s `lazyRetry`), `HelpBreadcrumb.tsx` (dòng "Hướng dẫn sử dụng › Module › Bài" — tín hiệu xác nhận đã điều hướng khi bấm HDSD từ popup ở chỗ khác trên màn hình; dùng `useHelpNavOptional` nên không bắt test phải bọc Provider), `help.css` (`animate-fade-in-once` khi đổi bài + `animate-help-flash` cho module vừa mở trong sidebar; cả 2 tôn trọng `prefers-reduced-motion`), `layouts/HelpNavTree.tsx` (cây dropdown 2 cấp trong sidebar — `AppShell.tsx`'s `NavItem`/`NavChildItem` chỉ hỗ trợ đúng 2 cấp nên bọc riêng, không generalize type dùng chung; **`<nav>` sidebar tự cuộn và mục HDSD nằm CUỐI danh sách nên cây thường mở ngoài khung nhìn** → `scrollIntoView({block:"nearest"})` vào phần tử active + flash nền, nếu không thì bấm HDSD trên desktop trông như không có gì xảy ra).
- Sửa file chung: `AppShell.tsx` (nhánh `it.id==="help"` trong items-map + `HdsdLink` trong header; `<main>` là scroll container duy nhất — `overflow-auto`), `MainPage.tsx` (`"help"` trong `ViewId`, state `helpModule`/`helpTopic`/`helpExpandedModuleId`, `HelpNavProvider`, case `renderActiveView()`, + effect reset `main.scrollTop` khi vào/đổi bài trong `"help"` — không có nó thì bấm HDSD lúc trang đang cuộn sâu sẽ render bài mới ngoài khung nhìn, trông như không có gì xảy ra; guard `typeof scrollTo === "function"` vì jsdom không implement), `components/ui/Modal.tsx` (+ prop `headerExtra?: ReactNode`, optional/không đổi behavior khi bỏ trống — dùng cho modal chèn HDSD mà không có chỗ cạnh `title`).
...
- Nghiệm thu: đã live-verify 19/23 điểm chèn trên sandbox thật (báo cáo: `docs/plans/QA_HDSD_NGHIEM_THU_2026-07-26.md`). Còn 4 điểm chưa trigger được vì cần state hiếm — 2 popup fail-case (nhắc xuất HĐ lỗi, số tiền không khớp ở CK ngoài) + 2 module-level ít rủi ro (bill viewer, bỏ xác nhận cộng buổi). Không phát hiện điểm nào vỡ layout hay đè nút đóng.
