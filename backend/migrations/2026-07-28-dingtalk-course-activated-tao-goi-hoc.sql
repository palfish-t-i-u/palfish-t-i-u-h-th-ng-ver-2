-- Migration: đổi thuật ngữ "Kích hoạt khóa học" → "Tạo gói học" trong tin DingTalk course_activated.
-- Chốt a Minh (chị Thu Hiền, nghiệp vụ) 28/7: PH mới là người kích hoạt khoá học thật (trên CRM);
-- thao tác Ops làm trong app chỉ là tạo/mở gói học — đổi chữ cho đúng nghĩa, tránh sale hiểu nhầm.
-- Live path = SQL fn build_course_activated_message (trigger trg_course_activated_dingtalk).
-- Python mirror utils/zalo_message_builder.build_course_activated_message đã đổi đồng bộ.
--
-- Format (giữ nguyên cấu trúc bản 2026-07-17, chỉ đổi dòng tiêu đề):
