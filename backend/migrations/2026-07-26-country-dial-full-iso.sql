-- Migration: đồng bộ public.format_phone_intl với bảng dial ISO đầy đủ
-- Lý do: bản cũ (2026-07-04-zalo-phone-intl-format.sql) chỉ CASE 10 nước → khách nước
--        ngoài (CZ, DE, GB...) rơi về '84' → SĐT trong tin Zalo/DingTalk hiện sai mã vùng.
--        Cùng lớp bug với nội dung CK (PR-2026-0578, 26/7/2026). Bản Python đã gộp về
--        backend/utils/country_dial.py — bản SQL này giữ ĐỒNG BỘ với file đó.
-- Phạm vi: CHỈ đổi ruột format_phone_intl (thay CASE bằng jsonb lookup đủ ISO alpha-2).
--          Signature + IMMUTABLE giữ nguyên → mọi hàm gọi nó (build_payment_paid_message,
--          build_course_activated_message...) tự hưởng, KHÔNG cần sửa các hàm đó.
-- Không backfill: tin đã render trong zalo_outbox là text tĩnh, không đổi hồi tố; chỉ tin
--          MỚI dùng map mới. Không đụng dữ liệu bảng nào.
-- Date: 2026-07-26
...
DECLARE
  v_digits TEXT;
  v_cc     TEXT;
  -- ISO 3166-1 alpha-2 → E.164 calling code (chỉ số). Giữ đồng bộ với
  -- backend/utils/country_dial.py COUNTRY_DIAL. NANP (US/CA/Caribbean) cùng '1'.
...
BEGIN
  ...
  -- country trống → 'VN'; không nhận diện → '84' (an toàn, hiếm khi rơi vào với map đủ ISO)
  v_cc := COALESCE(v_map ->> UPPER(COALESCE(NULLIF(TRIM(country), ''), 'VN')), '84');
