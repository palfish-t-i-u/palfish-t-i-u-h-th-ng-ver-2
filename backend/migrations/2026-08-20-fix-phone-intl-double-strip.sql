-- Migration: fix format_phone_intl strip dial nhầm khi SĐT local bắt đầu trùng dial code
-- Bug: Vinaphone 084xxx → local "844976431" (9 digits) bị strip "84" → "4976431" (7 digits)
-- Root cause: condition cũ `LENGTH > LENGTH(dial) + 5` quá lỏng (9 > 7 = TRUE)
-- Fix: thêm v_local_len map, chỉ strip khi afterDial ≈ expectedLocal (±1)
-- Scope: CHỈ đổi ruột format_phone_intl. Signature + IMMUTABLE giữ nguyên.
-- Impact: trigger Zalo/DingTalk tự hưởng. Tin đã render trong outbox là text tĩnh, không đổi.
-- Date: 2026-08-20
...
DECLARE
  ...
  -- Độ dài phần thuê bao sau dial code. Sync với backend/utils/country_dial.py COUNTRY_LOCAL_LEN.
  v_local_len_map CONSTANT jsonb := '{
...
BEGIN
  ...
  v_expected_local := COALESCE((v_local_len_map ->> UPPER(COALESCE(NULLIF(TRIM(country), ''), 'VN')))::int, 9);

  v_digits := regexp_replace(v_digits, '^0+', '');

  IF LEFT(v_digits, LENGTH(v_cc)) = v_cc AND LENGTH(v_digits) > v_expected_local THEN
    v_after_dial := SUBSTRING(v_digits FROM LENGTH(v_cc) + 1);
    IF ABS(LENGTH(v_after_dial) - v_expected_local) <= 1 THEN
      v_digits := v_after_dial;
    END IF;
  END IF;
