...
>
> ✅ **CẬP NHẬT 28/07/2026 (feedback anh Minh — mục lục + điểm chèn cấp hành động)** — 2 phần:
> 1. **Nhóm mục lục `/docs` theo khu vực nghiệp vụ**: thêm `HELP_MODULE_GROUPS` + `groupHelpModules()` trong `moduleLabels.ts`, dùng lại ĐÚNG 6 nhóm sidebar app thật (`MainPage.tsx:275-360`): Khách hàng & Đơn hàng / Đối soát & Hóa đơn / Báo cáo / Dữ liệu / Quản trị / Tài khoản & Quyền. Áp dụng cho cả `DocsSidebar.tsx` (section header tái dùng class CSS của `AppShell.tsx`) và `HelpLanding.tsx`. Có lưới an toàn "Khác" cho module lỡ quên map.
> 2. **Thêm điểm chèn cấp hành động nhỏ cho Sale + Kế toán** (ưu tiên theo yêu cầu): 3 bài mới — `paymentRequests/sua-thong-tin-khach-hang.md` (panel B1), `paymentRequests/quan-ly-lan-thanh-toan.md` (panel "Các lần thanh toán" — thêm lần TT bổ sung + huỷ/đánh dấu/sửa/refresh 1 dòng), `reconciliation/xem-anh-bill.md` (modal Album ảnh bill) — cả 3 chụp ảnh **read-only trên PR/giao dịch có sẵn trong sandbox** (không cần tạo test data, không cần revert). + 2 điểm chèn link-only: tab "CK ngoài chờ ghép" (`reconciliation/ghep-ck-ngoai` — bài đã có sẵn, trước đó chỉ link từ drawer con) và cross-link từ thanh đồng bộ mPOS/Payoo (`CardReconciliationTab.tsx`) sang `gatewaySync/cai-tien-ich`.
> - **Cố tình chưa làm**: `reconCard/so-tien-khong-khop-the.md` (mirror thẻ của `reconciliation/so-tien-khong-khop`) — lý do giống hệt bài gốc: không có nút tạo tay giao dịch thẻ lệch tiền, và quy tắc bắt buộc bài MỚI phải có ảnh thật (không được thêm vào `NO_SCREENSHOT_YET`). Giữ nguyên `<HdsdLink moduleSlug="reconCard" />` hiện tại (chưa có topic) — làm khi có giao dịch lệch tiền thật xuất hiện tự nhiên.
> - `tsc -b` + 659 unit test + `screenshots.test.ts` (54 bài, tất cả có ảnh) đều xanh.
