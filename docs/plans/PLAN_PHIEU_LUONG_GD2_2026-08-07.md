...
- **G2-N2** — Số ngày phép tồn hiện tại của từng người (đây là thông tin cá nhân), cùng quy tắc cộng dồn phép và thời điểm reset về không (cuối tháng hay cuối năm). Chỉ cần xin một lần để làm số dư đầu (xem G2-T3).
...
- **G2-N7** — Số người phụ thuộc của từng người để tính khoản giảm trừ gia cảnh (6,2 triệu/người). Hiện chị Vân tự tổng hợp, nên cần chị Vân cung cấp nguồn này.

## Danh sách công việc

> **Nguyên tắc xuyên suốt:** phần TÍNH nằm ở BigQuery (các view), Google Sheet chỉ **hiển thị kết quả cuối** và là nơi chị Trang/Vân soi và xác minh — không dựng lại công thức tính trên sheet (tránh một logic nằm hai nơi rồi lệch nhau; sau này lên app cũng đọc thẳng số từ BigQuery). Các khoản nhập tay (bù tiền, thưởng...) đưa vào nguồn trước khi lên BigQuery, không gõ thẳng lên sheet kết quả.
...
- **G2-T1** — Soạn một Google Sheet chấm công. Không cần tách ba tab: bản chất sheet chỉ để bắt các ca ngoại lệ, nên gộp phần *xác nhận công* (đi muộn, làm bù công, bù doanh số) và phần *xin nghỉ phép* vào chung một sheet "ChamCong", phân biệt bằng cột "loại" và cột "phòng ban" (sale khai qua leader xác nhận, back office chị Trang khai). Phần đầu nối thông tin cá nhân (số ngày phép còn lại...) chỉ cần khoá theo employeeID. Phép còn lại sẽ tự bù vào khi có đơn xin nghỉ.
- **G2-T2** — Đưa biểu mẫu trên cho chị Trang và các sale leader duyệt, chốt lại các cột cho đúng (gắn với mục G2-N1).
- **G2-T3** — Dựng phần tính phép: chỉ nhân viên chính thức mới có phép, phép được cộng dồn theo tháng và reset vào cuối kỳ. Chỉ cần xin chị Trang số phép tồn hiện tại **một lần duy nhất tháng này** làm số dư đầu; từ các tháng sau hệ thống tự cộng phép mới và trừ phép đã dùng bằng công thức, không phải xin lại.
...
- **G2-T7** — Minh viết logic đó thành **view tính trên BigQuery** (không dựng công thức trên sheet), lấy số liệu doanh số trực tiếp từ app GMV, và cho kết quả hoa hồng đổ thẳng vào bảng lương chứ không để ở một tab riêng.

### Milestone 3 — Gọn lại bảng lương và hoàn thiện phần thuế, bảo hiểm

- **G2-T8** — Làm bảng lương thành một sheet duy nhất chỉ để **hiển thị kết quả** đổ ra từ BigQuery (không chứa công thức tính). Toàn bộ dữ liệu của mọi tháng được chốt và lưu trong kho BigQuery; sheet chỉ hiển thị đúng tháng đang cần và xuất Excel của riêng tháng đó, nhờ vậy sheet luôn nhẹ. Sở dĩ bỏ cách để mỗi tháng một tab hoặc dồn tất cả vào một tab là vì sau khoảng ba tháng, số dòng lên hơn một nghìn khiến sheet nặng và khó thao tác.
- **G2-T9** — Công thức lương cứng viết trong **view BigQuery**: lương cứng chỉ phụ thuộc vào lương cơ bản ghi trong hợp đồng cộng với phần KPI vượt, không phụ thuộc lương cứng của tháng trước, nên logic tháng nào cũng giống nhau.
- **G2-T10** — Hoàn thiện phần thuế và bảo hiểm: bổ sung phần xác định ai là nhân viên chính thức để áp đúng tỷ lệ bảo hiểm (gắn với G2-N5), và áp mức ăn ca 730.000đ khi tính thuế theo bảng của chị Vân (gắn với G2-N6); số người phụ thuộc lấy từ nguồn của chị Vân để tính giảm trừ (gắn với G2-N7).
...
- **Minh** phụ trách phần code: biểu mẫu và sheet chấm công, đấu nối kho dữ liệu hai chiều, các view tính (hoa hồng, lương, thuế, bảo hiểm) trên BigQuery, bảng lương một sheet hiển thị, và tích hợp gửi phiếu.
...
- **Nguyên tắc kiến trúc:** BigQuery làm phần tính (các view), Google Sheet chỉ hiển thị kết quả cuối và là nơi xác minh — không dựng lại công thức tính trên sheet, tránh một logic nằm hai nơi rồi lệch nhau.
