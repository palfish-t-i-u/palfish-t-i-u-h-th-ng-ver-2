# Handoff — Minh nghỉ 2026-07-01..03

> Audit 2026-06-30. Migration Zalo bug `2026-06-29-zalo-msg-use-crm-name.sql` đã apply prod xong (`jozcvbbypwvzaefteoxn`). M3-01 đã đóng (bucket `tax_exports` không cần).
>
> Còn 5 việc, chia đều 3 người. Không cần Minh xử lý gì trước khi đi.

---

## ĐỨC — VAC-05: Cleanup fallback 3700 BE

**Bối cảnh:** Bảng `exchange_rates` đã sống (có data, có UI `ExchangeRatesPanel`, BE đã có `get_rate_for_date()` đọc bảng). Tuy nhiên 3 chỗ trong BE vẫn fallback cứng 3700 thay vì re-query bảng → khi bảng có rate ngày khác, các path này không tôn trọng.
...
## GIANG — VAC-02: Tách screen "Biến động số dư"

**Bối cảnh:** SePay webhook đẩy bank transactions realtime → `bank_transactions` table. Hiện tại kế toán chỉ thấy CK chưa khớp PR qua `ReconciliationTab` → sub-tab "CK ngoài chờ ghép". Thiếu screen tổng quan toàn bộ CK (khớp + chưa khớp + ignored).

**Scope:**

- Tạo tab/screen riêng "Biến động số dư" (sidebar mục mới hoặc sub-tab `ReconciliationTab`).
- Endpoint BE: `GET /api/v1/bank-transactions` (paginate + filter). Check `backend/sepay_routes.py` xem có sẵn chưa, nếu thiếu thì add.
- FE: list table `bank_transactions` với cột: thời gian, STK, số tiền, nội dung CK, match status (matched/pending/ignored), PR ID (nếu khớp).
- Filter: ngày, STK, số tiền, nội dung, status.
- Manual match → PR: tái dùng flow trong "CK ngoài chờ ghép" (drawer chọn PR + endpoint match thủ công).

**Tham khảo:**
- `docs/HANDOFF_GIANG_SEPAY_2026-06-16.md:140` — task gốc, ưu tiên "Cao — cần cho kế toán".
- `docs/HANDOFF_task8_casso.md` — FE design cũ thời Casso, tái dùng được.
- `backend/sepay_routes.py` + `backend/payment_request_routes.py:917` `reconcile_payment_line_webhook`.

---

## ĐẠT — VAC-03 + VAC-04 + VAC-06
...
Sau đó: `gh pr create --base main --head sandbox` với title `Merge sandbox: addr static JSON + handoff docs (T6/30)`.
...
Toàn team:
- Branch riêng cho mỗi task: `vac-05-rate-cleanup`, `vac-02-bank-screen`, `vac-03-installment-validate`, etc.
