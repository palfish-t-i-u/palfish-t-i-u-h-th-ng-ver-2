...
BEGIN
    ...
    SELECT 
        ...
        jsonb_build_object('count', count(*), 'ids', array_agg(p.id::text)) AS details
    FROM payments p
    ...
    FROM payments p
    WHERE p.sale_id IS NULL OR p.real_pay_vnd IS NULL OR p.team IS NULL
