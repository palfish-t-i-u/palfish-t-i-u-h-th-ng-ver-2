...
| 🛠️ **4-07 (mới 13/06)** | Default permission `sale` đang có `reconciliation: "full"` — sale ở mọi level lẽ ra KHÔNG được vào B2 Đối soát (anh Minh chốt), nhưng hiện đang có quyền nếu chưa override thủ công qua tab Phân quyền sử dụng. | (1) **DB**: trong cùng [`admin_routes.py:117-123`](backend/admin_routes.py:117) `DEFAULT_DEPT_PERMISSIONS["sale"]` → đổi `reconciliation: "full"` → `"none"`. (2) **Test**: login sale sandbox → sidebar không hiện "Đối soát giao dịch" nữa. (3) Bonus: bug 1B-01 + 1B-03 (FE catch im lặng confirm/reject transaction) tự khắc trở thành defense-in-depth vì sale không vào được B2 — pattern fix em đã đẩy trong `d495f67` vẫn đúng. | **BE + DB config** |
