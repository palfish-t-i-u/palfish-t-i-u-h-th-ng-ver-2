...
**Verify:** `grep -n "open && hasActiveRequest && activeRequest" frontend/src/components/payment-request/PaymentRequestDetailDrawer.tsx` — 1 hit, dòng kế có `key={activeRequest.id}`.
