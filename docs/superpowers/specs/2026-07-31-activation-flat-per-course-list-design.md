...
type CourseRow = {
  ...
  createdAt: string;
  uid: string;
  uidName?: string;     // ActiveUidGroup.name (multi-con) — hiện dưới UID nếu có
  courseCode: string;   // GIỮ NGẦM, không render (D4)
  packageName: string;
  amount: number;
...
Cột (gọn cho split-screen — D9). Bảng dùng `min-width` **scoped** (inline `style={{ minWidth: 900 }}` trên đúng `<table>` này, không đụng `.tbl` chung) để `.tbl-wrap { overflow-x:auto }` cho cuộn ngang.

| Cột | Nội dung |
|-----|----------|
| **UID** | `[⧉ copy]` UID · (dưới: tên bé nếu có) — D2, D4 |
| **Gói học** | `packageName` — D4 |
| **Khách hàng** | `customerName` (dưới: · Sale: tên) |
| **Tiền** | `vnd(amount)` (phải) |
| **Order ID** | `<input>` inline + `[Lưu]` + chip trạng thái khoá (⏳ Chờ / ✓ Đã KH) — D3, D7 |
| **AR** | AR-ID pill (bấm → mở drawer) · (dưới: Tạo lúc + PR-ID nhỏ) |

**Bỏ khỏi danh sách phẳng** (giảm bề ngang, vẫn còn nơi khác):
- **Course Code** → chỉ trong drawer (D4).
- **Cột "Thưởng GT"** → vẫn lọc được bằng chip "Thưởng giới thiệu" + xem trong drawer. (Referral là cấp-AR, lặp trên mỗi dòng-khoá gây rộng + thừa.)
- **Cột PR-ID độc lập** → chuyển thành dòng phụ nhỏ dưới AR-ID (D1 "không gom theo PR-ID").

**Row click** vẫn mở drawer (`setOpenArId(a.id)`) cho các tác vụ khác (cộng buổi giới thiệu, thêm gói, địa chỉ, xuất HĐ). Các control inline (input, nút Lưu, nút Copy) phải `stopPropagation` để không mở drawer khi thao tác.
...
State: `orderIdDrafts: Record<string, string>` (key = `row.key`). Input value = `drafts[key] ?? row.orderId`.

Nút **Lưu** bật khi: `draft.trim() !== "" && draft.trim() !== row.orderId && !đang-lưu`. Khi bấm:
1. **Đọc AR tươi từ state** `activeRequests.find(id)` (KHÔNG dùng object đã capture lúc render) — chặn ghi đè cũ.
2. `next = applyCourseOrderId(freshAr, courseCode, draft.trim())`.
3. `await persistActiveRequest(next)` — dùng nguyên đường có sẵn: cùng PATCH `active-requests`, cùng xử lý **409 trùng Order ID** (modal + `apiNote`), cùng `notifyLedgerChanged()` ghi Sổ doanh thu.
4. `ok` → xoá `drafts[key]` (input tự rơi về giá trị đã lưu), hiện "Đã lưu ✓" tạm thời. `!ok` → giữ draft, lỗi đã hiển thị sẵn.
