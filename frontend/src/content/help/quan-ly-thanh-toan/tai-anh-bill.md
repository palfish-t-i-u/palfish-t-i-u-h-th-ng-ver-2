---
title: "Tải ảnh bill & xử lý thiếu bill"
order: 40
audience: ["sale"]
---

Áp dụng khi: khách đã chuyển khoản, cần tải ảnh biên lai (bill) lên PR để kế toán đối soát.

## Tải ảnh bill

1. Mở PR → vào lần thanh toán cần gắn bill → bấm vào khu vực **"Tải ảnh bill"**.
2. Có 3 cách đưa ảnh vào:
   - **Dán ảnh (Ctrl+V)** — copy ảnh rồi dán thẳng vào khung.
   - **Kéo-thả** — kéo file ảnh từ máy tính vào khung.
   - **Chọn file** — bấm chọn ảnh từ máy tính (chọn được nhiều ảnh cùng lúc).
3. Ảnh tải lên hiện ngay trong khung, có thể tải thêm nhiều ảnh nếu cần.

## Khi hệ thống báo thiếu bill

Nếu bấm **Báo đơn & Kích hoạt** mà còn lần thanh toán nào chưa có ảnh bill, hệ thống sẽ chặn lại và hiện cảnh báo yêu cầu bổ sung bill trước khi tiếp tục.

1. Đọc cảnh báo để biết đúng lần thanh toán nào còn thiếu.
2. Đóng popup, quay lại lần thanh toán đó → tải ảnh bill như hướng dẫn ở trên.
3. Quay lại bấm **Báo đơn & Kích hoạt** lần nữa.

> ⚠️ Lưu ý: kế toán dùng ảnh bill để đối chiếu khi ghép chuyển khoản ngoài (tab **CK ngoài chờ ghép**) — thiếu bill sẽ làm chậm quá trình xác nhận tiền về, đặc biệt khi có nhiều lần TT cùng số tiền.
