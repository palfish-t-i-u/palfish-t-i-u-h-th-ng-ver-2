...
type PaymentFlowContextValue = {
  ...
  handleUpdatePaymentRequest: (id: string, payload: UpdatePaymentRequestPayload) => Promise<PaymentRequest>;
...
export function PaymentFlowProvider({
  ...
  const handleUpdatePaymentRequest = useCallback(
    async (id: string, payload: UpdatePaymentRequestPayload) => {
      ...
      const fromApi = fromApiPaymentRequest(response.data.payment_request);
      let saved: PaymentRequest | null = null;
      setRequests((prev) =>
        prev.map((r) => {
          if (r.id !== id) return r;
          const payments = fromApi.payments.length > 0 ? fromApi.payments : r.payments;
          saved = normalizeRequest({ ...r, ...fromApi, payments });
          return saved;
        })
      );
      setApiNote("");
      return saved ?? normalizeRequest(fromApi);
    },
    []
  );
