...
/** PATCH /api/v1/payment-requests/{id} — snake_case body (matches PaymentRequestUpdate on BE) */
export type UpdatePaymentRequestPayload = {
  /** Required when sent — empty string rejected by BE */
  uid?: string;
  name?: string;
  phone?: string;
  /** Sent as "" to clear optional text columns on BE */
  country?: string;
  address?: string;
  ward?: string;
  province?: string;
