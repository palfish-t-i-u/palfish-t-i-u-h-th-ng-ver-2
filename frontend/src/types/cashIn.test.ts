...
function _rawSample(): CashInReportRaw {
  return {
    summary: {
      ...
      rate: 3700, unsynced_settlement_count: 2, unsynced_settlement_amount: 12_345_000,
    },
...
describe("mapCashInReport — snake_case (BE) → camelCase (FE)", () => {
  it("map summary đúng field", () => {
    ...
    expect(out.summary).toEqual({
      ...
      unsyncedSettlementCount: 2, unsyncedSettlementAmount: 12_345_000,
    });
  });
  ...
  it("map rows giữ nguyên txn_id → txnId, business_line → businessLine, không mất field nào", () => {
    ...
    expect(out.rows[0]).toEqual({
      ...
      mainCat: null, detail: null, isSplit: false, unmatched: false,
    });
  });
});
