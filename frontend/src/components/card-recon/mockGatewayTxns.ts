...
// ── Trạng thái tiện ích đồng bộ (mock FE) ──
// Thực tế: extension sẽ nhét cờ "đã cài" vào trang app. Ở đây dùng localStorage để demo state-aware.
export const EXT_FLAG_KEY = "gw_ext_installed";

export function isExtInstalled(): boolean {
  try {
    return localStorage.getItem(EXT_FLAG_KEY) === "1";
  } catch {
    return false;
  }
}

export function setExtInstalled(v: boolean): void {
  try {
    if (v) localStorage.setItem(EXT_FLAG_KEY, "1");
    else localStorage.removeItem(EXT_FLAG_KEY);
  } catch {
